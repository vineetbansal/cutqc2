...
class AbstractGraphContractor(ABC):
    ...
    def reconstruct(
        self, compute_graph: ComputeGraph, subcircuit_entry_probs: dict, num_cuts: int
    ) -> None:
    ...
    def _set_smart_order(self) -> None:
        ...
        for subcircuit_idx in self.subcircuit_entry_probs:
            first_entry_init_meas = list(
                self.subcircuit_entry_probs[subcircuit_idx].keys()
            )[0]
            length = len(
                self.subcircuit_entry_probs[subcircuit_idx][first_entry_init_meas]
            )
        ...
        self.subcircuit_entry_lengths = [
            subcircuit_entry_lengths[i] for i in self.smart_order
        ]
