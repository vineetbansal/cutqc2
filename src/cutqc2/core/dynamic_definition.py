...
class DynamicDefinition:
    ...
    def _build(self):
        ...
        while recursion_layer < self.recursion_depth:
            ...
            for subcircuit_idx in merged_subcircuit_entry_probs:
                first_entry_init_meas = list(
                    merged_subcircuit_entry_probs[subcircuit_idx].keys()
                )[0]
                length = len(
                    merged_subcircuit_entry_probs[subcircuit_idx][first_entry_init_meas]
                )
            ...
            for edge_bases in itertools.product(
                ["I", "X", "Y", "Z"], repeat=len(edges)
            ):
                self.compute_graph.assign_bases_to_edges(
                    edge_bases=edge_bases, edges=edges
                )
                summation_term = []
                for subcircuit_idx in smart_order:
                    ...
                    subcircuit_entry_prob = merged_subcircuit_entry_probs[
                        subcircuit_idx
